    ...
    public class JsonMapper
    {
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                while (true) {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
						...
						{
#if UNITY_EDITOR
							string text = string.Format("not found at data Preperty = {0}\n", reader.Value);
#endif
							reader.Read();
							if (reader.Token == JsonToken.ObjectStart)
								invalidObjCount++;
#if UNITY_EDITOR
							text = string.Format("{0} Value = {1}", text, reader.Value);
        ...
        public static string ToJson (object obj)
        {
            lock (static_writer_lock) {
				...
				string strInfo = static_writer.ToString();
				static_writer = new JsonWriter();
				return strInfo;
            }
        }
